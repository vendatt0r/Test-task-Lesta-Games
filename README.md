...
- Работает для всех целых чисел, включая отрицательные.
- Эффективно работает с типами данных int и bool, и не требует дополнительных преобразований данных.

### Минусы:
- Операция взятия остатка является арифметической и требует больше вычислительных ресурсов, чем простая побитовая операция.
